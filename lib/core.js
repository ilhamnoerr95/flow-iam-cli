...
// sprint finish
function sprintFinish(targetBranch = "main") {
  ...
  if (!targetBranch) {
    console.log("❌ Target branch is required (e.g. main / master)");
    return;
  }
...
function flowIam() {
  ...
  prompt("👉 Input your choice [1-12]: ", (c) => {
    ...
    if (c == 8) prompt("Input your target branch: ", (b) => sprintFinish(b));
...
export function run(argv) {
  ...
  switch (cmd) {
    ...
    case "-sf":
    case "sprint-finish":
      prompt("Input your target branch: ", (b) => sprintFinish(b));
      break;
